...
if __name__ == '__main__':
    ...
    query_word = urllib.parse.quote_plus(
        arxiv_url + " AND -filter:retweets AND -filter:replies")

    url = "https://api.twitter.com/1.1/search/tweets.json?q=" + query_word
    auth = OAuth1(consumer_key, consumer_key_secret, access_token,
                  access_token_secret)
    ...
    for tweet in data:
        ...
        splitted_text_without_title = [
            x for x in splitted_text if x not in arxiv_title.split()
        ]
